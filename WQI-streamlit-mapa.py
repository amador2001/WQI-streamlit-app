...
st.set_page_config(
	page_title="Water Quality Index Modeling",
	page_icon="",
	layout="wide",
...
# if st.sidebar.button("Predict2", key="1")==False:
#     filtered_data2 = data_map[data_map["WQI"] > 92]
#     map_holder= st.map(filtered_data2)
...
#filtered_data2 = data_map[data_map["WQI"] > 92]
filtered_data2 = data_map.drop_duplicates(subset = ["Station"]).iloc[0:30]
map_holder= st.map(filtered_data2)

#-------------------------- fin 2  MAPA -----------------

#import plotly.express as px
#fig_2 = px.scatter(data, x="WQI pH", y="WQI")
#fig_2.show()
